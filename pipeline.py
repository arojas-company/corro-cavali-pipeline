...
def _smartrr_is_active(subscription):
    ...
    # If the API endpoint was already filtered by ACTIVE and no status is returned, count it.
...
def fetch_smartrr_active_subs(brand_name):
    ...
    This runs server-side in GitHub Actions so the API key stays secret.
    """
    ...
    source = "Smartrr API → GitHub Actions → Sheets"

    try:
        # Keep /subscription first because that is what the existing dashboard used.
        ...
        for endpoint in endpoints:
            ...
            while True:
                ...
                if not items:
                    # If filtered endpoint returns empty, retry next endpoint.
                    break
