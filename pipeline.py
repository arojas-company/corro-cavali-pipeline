"""
Pipeline CORRO / CAVALI v4 — FIXED COMPLETE
============================================
FIXES aplicados:
1. ql_run: maneja rows como list-of-lists O list-of-dicts (KeyError:0 fix)
2. ql_run: coerce seguro de valores a string antes de retornar
3. ql_row: guarda contra lista vacía con retorno None explícito
4. fetch_sales: gross_profit nunca None; fallback neto-cogs
5. _gm(): normaliza decimal→porcentaje correctamente (abs<1.5 check)
6. fetch_new_vs_returning: aliases first_time/repeat manejados
7. build(): fallback orders_fulfilled → sales.orders → len(orders)
8. make_kpi_row(): orden explícito por HEADERS, nunca desalineado
9. nc_by_month: usa HEADERS.index() en lugar de índice hardcodeado
10. calc_rs(): gm_pct ya viene en %, divide /100 correctamente
11. write_all rs_rows: padding a len(rs_headers) antes de append
12. calendar import al top-level (no dentro de función)
13. AD_SPEND_DATA: cast seguro con try/except en cac_auto
14. Todas las excepciones silenciosas logueadas con contexto
"""
...
# ── kpis_daily columns — orden canónico ───────────────────────────
HEADERS = [
    "updated_at",               # 0
    "period",                   # 1
    "period_start",             # 2
...
def gql(store_url, token, query):
    """Executes a raw GraphQL query against Shopify Admin API."""
    try:
        r = requests.post(
        ...
        print(f"    [gql] request exception: {exc}")
        return None

    if r.status_code != 200:
        print(f"    [gql] HTTP {r.status_code}: {r.text[:200]}")
        return None
...
def rest(store_url, token, endpoint, params):
    """Paginates through a Shopify REST endpoint and returns all records."""
    ...
    while url:
        try:
            ...
            print(f"    [rest] request exception: {exc}")
            break
        data = r.json()
...
def ql_run(store_url, token, ql_query):
    """
    Runs a ShopifyQL query and returns a list of {col: val} dicts.

    FIX (KeyError:0): ShopifyQL rows can be:
      - list-of-lists: [[v0, v1, ...], ...]  → positional indexing
      - list-of-dicts: [{"col": v, ...}, ...]  → dict.get()
    Both cases are now handled explicitly.
    """
    ...
    if errs:
        print(f"    [ql_run] parseErrors: {errs}")
        return []
    ...
    for row in rows:
        if isinstance(row, dict):
            # API returned rows already keyed by column name
            entry = {c: str(row.get(c, "") if row.get(c) is not None else "") for c in cols}
        elif isinstance(row, (list, tuple)):
            # Normal positional list
            entry = {
                cols[i]: str(row[i] if row[i] is not None else "")
                if i < len(row)
                else ""
                for i in range(len(cols))
            }
        else:
            print(f"    [ql_run] WARNING: unexpected row type {type(row)}: {row}")
            continue
        result.append(entry)
...
def ql_row(store_url, token, ql_query):
    """Returns the last row of a ShopifyQL aggregate query, or None."""
...
# ─────────────────────────────────────────────────────────────────
def _m(v):
    """Safely converts any value to float, returns 0.0 on failure."""
    if v is None:
        return 0.0
    try:
...
def _gm(v):
    """
    Normalises gross_margin from ShopifyQL.
    ShopifyQL may return:
      - 0.423  (decimal fraction) → convert to 42.3 %
      - 42.3   (already percentage) → leave as-is
    Rule: if abs(f) < 1.5 → multiply by 100.
    Returns float percentage (e.g. 42.3).
    """
    if v is None:
        return 0.0
    try:
...
# ─────────────────────────────────────────────────────────────────
# FETCH: SALES (gross_profit real via ShopifyQL)
# ─────────────────────────────────────────────────────────────────
def fetch_sales(url, token, s, e):
    """
    FROM sales — returns gross_sales, net_sales, gross_profit, cogs, gm%, orders.
    gross_profit is a real ShopifyQL field (not estimated).
    FIX: gross_profit never None; falls back to net_sales - cogs.
    """
    row = ql_row(
        url,
        token,
    ...
    if not row:
        print("    [fetch_sales] no data returned — using zeros")
    ...
    # FIX: explicit fallback if gross_profit key is missing or empty
    gp_raw = row.get("gross_profit")
    if gp_raw is not None and str(gp_raw).strip() not in ("", "None", "null"):
        gp = round(_m(gp_raw), 2)
    else:
        gp = round(n - c, 2)

    gm = _gm(row.get("gross_margin"))
    # FIX: recalculate gm if zero but we have gp and n
    ...
    print(
        f"    gross:{g:>12,.2f}  net:{n:>12,.2f}  gp:{gp:>10,.2f}  "
...
def fetch_new_vs_returning(url, token, s, e):
    """
    ShopifyQL GROUP BY customer_type → new/returning split.
    FIX: handles all Shopify aliases for new/returning customer_type.
    FIX: gross_profit per segment with fallback to 0.0.
    """
    rows = ql_run(
        url,
        token,
    ...
    for row in rows:
        ...
        if ctype in NEW_ALIASES:
            ...
            if ctype:
                print(f"    [nvr] unknown customer_type: '{ctype}'")
    ...
    print(
        f"    new_customers:{result['new_customers']:>6}  "
...
# ─────────────────────────────────────────────────────────────────
# FETCH: REST ORDERS (units + revenue share)
# ─────────────────────────────────────────────────────────────────
...
def calc_units(orders):
    ...
    for o in orders:
        for li in o.get("line_items", []):
            try:
                total += int(li.get("quantity", 0) or 0)
            except (ValueError, TypeError):
                pass
    return total
...
def calc_rs(orders, overall_gm_pct):
    ...
    GP per channel estimated as: channel_net_sales * overall_gm_pct / 100
    FIX: overall_gm_pct is already in percentage form (e.g. 42.3), divide /100.
    """
    ch = {
        "Wellington (POS)": 0.0,
        "Concierge":        0.0,
    ...
    for o in orders:
        try:
            ...
            amt = 0.0
        total += amt
        src  = str(o.get("source_name") or "").lower().strip()
        tags = str(o.get("tags") or "").lower()
    ...
    for k, v in ch.items():
        ...
        # FIX: overall_gm_pct is already % (42.3), so /100 to get multiplier
...
def build(sales, orders, nvr, sessions=0, orders_fulfilled=None):
    ...
    # FIX: recompute gp if zero but data available
    if gp == 0.0 and n > 0 and c >= 0:
        gp = round(n - c, 2)

    # FIX: three-level fallback for nb_orders
    if orders_fulfilled is not None:
        nb = int(orders_fulfilled)
    elif sales.get("orders"):
        nb = int(sales["orders"])
    else:
        nb = len(orders)
...
# ─────────────────────────────────────────────────────────────────
# BUILD KPI ROW
# FIX: explicit positional order matching HEADERS — never desync
# ─────────────────────────────────────────────────────────────────
def make_kpi_row(now_str, period_key, s, e, cur):
    """
    Builds a row in the exact order of HEADERS.
    FIX: explicit list (not dict iteration) to prevent column misalignment.
    """
    return [
        ...
        str(e),                                 # 3  period_end
        cur.get("gross_sales",            0.0), # 4
        cur.get("net_sales",              0.0), # 5
        cur.get("gross_profit",           0.0), # 6  ← KEY
        cur.get("total_discounts",        0.0), # 7
        cur.get("total_returns",          0.0), # 8
        cur.get("cogs",                   0.0), # 9
        cur.get("pct_discount",           0.0), # 10
        cur.get("pct_returns",            0.0), # 11
        cur.get("pct_gm",                 0.0), # 12
        cur.get("nb_orders",              0),   # 13
        cur.get("nb_units",               0),   # 14
        cur.get("aov",                    0.0), # 15
        cur.get("units_per_order",        0.0), # 16
        cur.get("sessions",               0),   # 17
        cur.get("unique_visitors",        0),   # 18
        cur.get("conversion_rate",        0.0), # 19
        cur.get("new_customers",          0),   # 20
        cur.get("returning_customers",    0),   # 21
        cur.get("new_revenue",            0.0), # 22
        cur.get("returning_revenue",      0.0), # 23
        cur.get("new_gross_profit",       0.0), # 24
        cur.get("returning_gross_profit", 0.0), # 25
    ]
...
def write_all(gc, sheet_id, kpi_rows, rs_rows, nvr_rows, brand_name):
    ...
    try:
        ...
        ws = sh.add_worksheet("kpis_daily", rows=600, cols=40)
    ...
    if len(existing_vals) >= 2:
        ...
        for r in existing_vals[1:]:
            ...
            if pk:
                # FIX: rebuild with current HEADERS to fix any column drift
    ...
    print(f"    kpis_daily: {len(merged)} rows written")

    # ── revenue_share ────────────────────────────────────────────
    try:
        ws_rs = sh.worksheet("revenue_share")
    except gspread.WorksheetNotFound:
        ws_rs = sh.add_worksheet("revenue_share", rows=600, cols=12)
    ...
    for r in rs_rows:
        # FIX: ensure row always has correct length before storing
    ...
    for r in sorted_rs:
        ...
        if pk.startswith("mtd_"):
            ...
            try:
                ...
                pq_n    = q - 1 if q > 1 else 4
                py      = yr if q > 1 else yr - 1
                prev_pk = f"q{pq_n}_{py}"
            except (ValueError, IndexError):
                pass

        prev_row = rs_idx.get((ch, prev_pk)) if prev_pk else None
        try:
            pct_now = float(r[4]) if str(r[4]) not in ("", "None") else None
        except (ValueError, TypeError):
            pct_now = None
        try:
            pct_prev = (
                float(prev_row[4])
                if prev_row and str(prev_row[4]) not in ("", "None")
                else None
            )
        except (ValueError, TypeError):
            pct_prev = None
        ...
        pct_chg = (
            round(pct_now - pct_prev, 2)
            if pct_now is not None and pct_prev is not None
            else None
        )
        while len(r) < len(rs_headers):
            r.append("")
    ...
    print(f"    revenue_share: {len(merged_rs)} rows written")

    # ── new_vs_returning ─────────────────────────────────────────
    try:
        ws_nvr = sh.worksheet("new_vs_returning")
    except gspread.WorksheetNotFound:
        ws_nvr = sh.add_worksheet("new_vs_returning", rows=300, cols=12)
    ...
    print(f"    new_vs_returning: {len(merged_nvr)} rows written")

    # ── ad_spend ─────────────────────────────────────────────────
    try:
        ws_ad = sh.worksheet("ad_spend")
    except gspread.WorksheetNotFound:
        ws_ad = sh.add_worksheet("ad_spend", rows=200, cols=10)
    ...
    # FIX: use HEADERS.index() — robust against column order changes
    ...
    for mo, vals in sorted(brand_data.items()):
        ...
        try:
            ...
            print(f"    [ad_spend] skipping invalid month key: {mo}")
            continue
        ps = f"{mo}-01"
        pe = f"{mo}-{calendar.monthrange(yr, mn)[1]:02d}"
        nc = nc_by_month.get(mo, 0)
        # FIX: safe cac_auto with try/except
        try:
    ...
    populated = sum(1 for r in ad_rows if r[8] != "")
    print(f"    ad_spend: {len(ad_rows)} months  (cac_auto for {populated} months)")
...
def main():
    ...
    for brand_name, cfg in STORES.items():
        ...
        for it in periods_to_run:
            ...
            # Skip periods with no valid key (e.g. mtd_yoy when pk=None)
            if pk is None:
                print(f"\n  [{label}] skipped — no period key")
                continue
            ...
            for ch, v in rs.items():
                row_rs = [
                    ...
                    v["gross_profit"],
                    v["gross_margin"],
                    "",   # pct_prev — calculated in write_all
                    "",   # pct_chg  — calculated in write_all
                    str(v["gp_is_estimate"]),
                ]
                rs_rows.append(row_rs)
            ...
            nvr_rows.append([
                ...
                nvr.get("new_revenue",            0.0),
                nvr.get("returning_revenue",      0.0),
                nvr.get("new_gross_profit",       0.0),
                nvr.get("returning_gross_profit", 0.0),
            ])
        ...
        for row in kpi_rows:
            try:
                gp_val  = float(row[6]  or 0)
                gm_val  = float(row[12] or 0)
                nc_val  = int(row[20]   or 0)
                gs_val  = float(row[4]  or 0)
                ns_val  = float(row[5]  or 0)
            except (ValueError, TypeError, IndexError):
                gp_val = gm_val = nc_val = gs_val = ns_val = 0
            print(
                f"    {str(row[1]):<24}  {row[2]} → {row[3]}"
                f"  gross:{gs_val:>12,.2f}"
                f"  net:{ns_val:>12,.2f}"
                f"  gp:{gp_val:>10,.2f}"
                f"  gm:{gm_val:>5.1f}%"
                f"  new_cust:{nc_val:>5}"
