...
SMARTRR_VOLUME_HEADERS = [
    "updated_at", "brand", "period", "period_start", "period_end",
    "product_variant", "sku", "total_quantity", "gross_revenue", "source", "error",
]
...
def write_smartrr(gc, sheet_id, smartrr_row):
    ...
    ws.clear()
    ws.append_row(["updated_at","brand","period","period_start","period_end","product_variant","sku","total_quantity","gross_revenue","source","error","example"] )
...
def _line_product_label(li):
    """Return the real product/variant label from a Shopify order line item.
    This avoids reading customer names from Smartrr purchase-state payloads.
    """
    raw = str(
        li.get("title") or li.get("product_title") or li.get("name") or li.get("variant_title") or ""
    ).strip()
    ...
    # Canonical labels as seen in Smartrr Advanced Analytics.
    if "cavali club junior membership" in t or "junior membership" in t:
        return "Cavali Club Junior Membership"
    if "signature box subscription" in t or "the signature box" in t or "signature box" in t:
        return "The Signature Box Subscription"
    if "premier box subscription" in t or "the premier box" in t or "premier box" in t:
        return "The Premier Box Subscription"
    if "cavali club membership" in t:
        return "Cavali Club Membership"
    if t in ("ø", "0") or raw == "ø":
        return "Ø"
    if "welcome box" in t:
        return "Welcome Box"
...
def _line_gross_revenue(li):
    """Gross line revenue, matching Smartrr Sales Volume style: unit price × quantity before order-level grouping."""
    qty = int(float(li.get("quantity") or 0))
    price = 0.0
    for key in ("price", "pre_tax_price", "discounted_price"):
        try:
            val = li.get(key)
            if val not in (None, ""):
...
def _line_product_label(li):
    """Return the real product/variant label from a Shopify order line item. Do not bucket to Other."""
    raw = str(li.get("title") or li.get("product_title") or li.get("name") or li.get("variant_title") or "").strip()
    return raw or "ø"


def _line_gross_revenue(li):
    """Gross line revenue: unit price × quantity."""
    qty = int(float(li.get("quantity") or 0))
    price = 0.0
    for key in ("price", "pre_tax_price", "discounted_price"):
        try:
...
def calc_smartrr_sales_volume_from_orders(orders, now_str, brand_name, period_key, period_start, period_end):
    """Build period/product rows for Cavali section 06.
    Period-specific only. Uses product names exactly from order line_items; no lifetime/latest fallback and no Other mapping.
    """
    if brand_name != "cavali":
        return []

    source = "Shopify order line_items · period-specific product volume for Smartrr section"
    agg = {}
    for o in orders or []:
        for li in o.get("line_items", []) or []:
            ...
            if qty <= 0:
                continue
            product = _line_product_label(li)
            sku = str(li.get("sku") or li.get("variant_id") or "ø").strip() or "ø"
            key = (product, sku)
            if key not in agg:
                agg[key] = {"qty": 0, "gross": 0.0}
            agg[key]["qty"] += qty
            agg[key]["gross"] = round(agg[key]["gross"] + _line_gross_revenue(li), 2)

    rows = []
    for (product, sku), vals in sorted(agg.items(), key=lambda kv: (-kv[1]["qty"], kv[0][0].lower())):
        rows.append([
            now_str, brand_name, period_key, str(period_start), str(period_end),
            product, sku, int(vals["qty"]), round(vals["gross"], 2), source, ""
        ])
    return rows

# Legacy Smartrr purchase-state helpers intentionally kept above for active-subscriber diagnostics,
# but the dashboard product-volume section uses calc_smartrr_sales_volume_from_orders().

def write_smartrr_product_rows(gc, sheet_id, smartrr_rows):
    if smartrr_rows is None:
        smartrr_rows = []
    sh = gc.open_by_key(sheet_id)
    try:
        ws = sh.worksheet("smartrr_product_volume")
    except Exception:
        ws = sh.add_worksheet("smartrr_product_volume", rows=max(100, len(smartrr_rows)+10), cols=len(SMARTRR_VOLUME_HEADERS))
    ws.clear()
    ws.append_row(SMARTRR_VOLUME_HEADERS)
    if smartrr_rows:
...
def main():
    ...
    print("Safe mode: tabs are NOT cleared at start. Each brand is replaced only after rows are rebuilt.", flush=True)

    for brand, cfg in STORES.items():
        print(f"\n{'='*55}\n  {brand.upper()} — BACKFILL 2024-01 → {today}\n{'='*55}")
        url, token, sid = cfg["url"], cfg["token"], cfg["sheet_id"]
        kpi_rows, rs_rows, nvr_rows, smartrr_rows = [], [], [], []
        ...
        for y in range(2024, today.year + 1):
            ...
            for m in range(m_start, m_end + 1):
                ...
                if brand == "cavali":
                    smartrr_rows.extend(calc_smartrr_sales_volume_from_orders(o_cur, now_str, brand, period_label, mo_start, mo_end))
                    smartrr_rows.extend(calc_smartrr_sales_volume_from_orders(o_mtd, now_str, brand, mtd_label, mo_start, mtd_end))
        ...
        while wk_start <= today:
            ...
            if brand == "cavali":
                smartrr_rows.extend(calc_smartrr_sales_volume_from_orders(o_cur, now_str, brand, wk_label, wk_start, wk_end))
        ...
        for y in range(2024, today.year + 1):
            ...
            for q in range(1, max_q + 1):
                ...
                if brand == "cavali":
                    smartrr_rows.extend(calc_smartrr_sales_volume_from_orders(o_q, now_str, brand, q_label, q_start, q_end))
        ...
        # Replace tabs only after this brand's rows are fully rebuilt.
        ws.clear()
        ...
        if brand == "cavali":
            ...
            for rr in smartrr_rows:
                totals[rr[5]] = totals.get(rr[5], 0) + int(float(rr[6] or 0))
            print(f"    smartrr sales volume rows: {len(smartrr_rows)}")
            for prod, cnt in sorted(totals.items(), key=lambda kv: -kv[1])[:10]:
                print(f"      smartrr sales volume test: {prod}={cnt}")
