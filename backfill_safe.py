...
def calc_smartrr_sales_volume_from_orders(orders, now_str, brand_name, period_key, period_start, period_end):
    """Build period/product rows for the Cavali Smartrr section.

    Smartrr Advanced Analytics 'Sales Volume by Product-Variant' is an order-line report.
    The earlier purchase-state endpoint is an active-subscription snapshot, so it cannot match
    date-filtered sales volume and can accidentally surface customer names. This mirrors that
    Smartrr report from Shopify order line items for the selected period.
    """
    if brand_name != "cavali":
        return []

    source = "Shopify order line items · Smartrr Sales Volume mirror"
    agg = {}
    examples = {}
    ...
    for o in orders or []:
        for li in o.get("line_items", []) or []:
            ...
            sku = str(li.get("sku") or "").strip()
            gross = _line_gross_revenue(li)
            key = (product, sku)
            if key not in agg:
                agg[key] = {"qty": 0, "gross": 0.0}
            agg[key]["qty"] += qty
            agg[key]["gross"] = round(agg[key]["gross"] + gross, 2)
            examples.setdefault(key, str(li.get("name") or li.get("title") or product)[:160])

    rows = []
    if not agg:
        rows.append([now_str, brand_name, period_key, str(period_start), str(period_end), "Other / Unmapped", 0, 0, "", source, "No product line items found for this period", ""])
        return rows

    for (product, sku), vals in sorted(agg.items(), key=lambda kv: (-kv[1]["qty"], kv[0][0])):
        rows.append([
            now_str, brand_name, period_key, str(period_start), str(period_end),
            product, vals["qty"], vals["gross"], sku, source, "", examples.get((product, sku), "")
        ])
    return rows
